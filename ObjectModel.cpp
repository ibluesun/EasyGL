...
void ObjectModel::LoadGLMFile(LPSTR filename)
{
	glmMesh.glmReadOBJ(filename);
...
void ObjectModel::Render()
{
	...
	glPushMatrix();
		...
		{
			glmMesh.glmDraw(GLM_SMOOTH + GLM_MATERIAL);
		}
		...
		if (modelTexture!=NULL)
		{
			...
			glmMesh.glmDraw(GLM_SMOOTH + GLM_MATERIAL + GLM_TEXTURE);
...
void ObjectModel::Unitize()
{
	glmMesh.glmUnitize();
}

void ObjectModel::Scale(GLfloat sc)
{
	glmMesh.glmScale(sc);
}

void ObjectModel::ReverseWinding()
{
	glmMesh.glmReverseWinding();
}

void ObjectModel::ChangeOpacity(GLdouble cc)
{
	Opacity=cc;
	glmMesh.opac=Opacity;
}
 ...
 */
GLfloat ObjectModel::Glm::glmUnitize()
{
    GLuint  i;
 ...
 */
GLvoid ObjectModel::Glm::glmDimensions(GLfloat* dimensions)
{
    GLuint i;
    GLfloat maxx, minx, maxy, miny, maxz, minz;
 ...
 */
GLvoid ObjectModel::Glm::glmScale(GLfloat scale)
{
    GLuint i;
 ...
 * model - properly initialized GLMmodel structure 
 */
GLvoid ObjectModel::Glm::glmReverseWinding()
{
    GLuint i, swap;
 ...
 * model - initialized GLMmodel structure
 */
GLvoid ObjectModel::Glm::glmFacetNormals()
{
    GLuint  i;
    GLfloat u[3];
    GLfloat v[3];
 ...
 */
GLvoid ObjectModel::Glm::glmVertexNormals(GLfloat angle)
{
    GLMnode*    node;
    GLMnode*    tail;
    GLMnode** members;
 ...
 */
GLvoid ObjectModel::Glm::glmLinearTexture()
{
    GLMgroup *group;
 ...
 * model - pointer to initialized GLMmodel structure
 */
GLvoid ObjectModel::Glm::glmSpheremapTexture()
{
    GLMgroup* group;
 ...
 * model - initialized GLMmodel structure
 */
GLvoid ObjectModel::Glm::glmDelete()
{
    GLMgroup* grp;
    GLuint i;
 ...
 */
void ObjectModel::Glm::glmReadOBJ(char* filename)
{
    FILE*   file;
 ...
 */
GLvoid ObjectModel::Glm::glmWriteOBJ(char* filename, GLuint mode)
{
    GLuint  i;
    FILE*   file;
    GLMgroup* group;
...
//    assert(model);
    ...
    if (mode & GLM_MATERIAL && model->mtllibname) {
        fprintf(file, "\nmtllib %s\n\n", model->mtllibname);
        glmWriteMTL(filename, model->mtllibname);
    }
 ...
 *             GLM_FLAT and GLM_SMOOTH should not both be specified.  
 */
GLvoid ObjectModel::Glm::glmDraw(GLuint mode)
{
    static GLuint i;
    static GLMgroup* grp;
...
//    assert(model);
    ...
    while (grp) {
        if (mode & GLM_MATERIAL) {
            material = &model->materials[grp->material];

			material->ambient[3]=opac;
			material->diffuse[3]=opac;
			material->specular[3]=opac;
 ...
 * GLM_FLAT and GLM_SMOOTH should not both be specified.  
 */
GLuint ObjectModel::Glm::glmList(GLuint mode)
{
    GLuint list;
 ...
 *
 */
GLvoid ObjectModel::Glm::glmWeld(GLfloat epsilon)
{
    GLfloat* vectors;
    GLfloat* copies;
    GLuint   numvectors;
...
GLfloat ObjectModel::Glm::glmMax(GLfloat a, GLfloat b) 
{
	if (b > a)
		return b;
	return a;
...
GLfloat ObjectModel::Glm::glmAbs(GLfloat f)
{
	if (f < 0)
		return -f;
	return f;
 ...
 */
GLfloat ObjectModel::Glm::glmDot(GLfloat* u, GLfloat* v)
{
	assert(u); assert(v);
 ...
 */
GLvoid ObjectModel::Glm::glmCross(GLfloat* u, GLfloat* v, GLfloat* n)
{
 ...
 */
GLvoid ObjectModel::Glm::glmNormalize(GLfloat* v)
{
	GLfloat l;
 ...
 * v - array of 3 GLfloats (GLfloat v[3]) 
 */
GLboolean ObjectModel::Glm::glmEqual(GLfloat* u, GLfloat* v, GLfloat epsilon)
{
 ...
 *
 */
GLfloat* ObjectModel::Glm::glmWeldVectors(GLfloat* vectors, GLuint* numvectors, GLfloat epsilon)
{
	GLfloat* copies;
	GLuint   copied;
	GLuint   i, j;
...
void ObjectModel::Glm::glmFindGroup(char* name)
{
	GLMgroup* grp;
...
void ObjectModel::Glm::glmAddGroup(char* name)
{
	GLMgroup* grp;
	...
	if (!grp) {
		grp = (GLMgroup*)malloc(sizeof(GLMgroup));
		grp->name = strdup(name);
		grp->material = 0;
		grp->numtriangles = 0;
		grp->triangles = NULL;
		grp->next = model->groups;
		model->groups = grp;
		model->numgroups++;
	}
...
GLuint ObjectModel::Glm::glmFindMaterial(char* name)
{
	GLuint i;
 ...
 */
char* ObjectModel::Glm::glmDirName(char* path)
{
	char* dir;
	char* s;
 ...
 */
GLvoid ObjectModel::Glm::glmReadMTL(char* name)
{
	FILE* file;
	char* dir;
	char* filename;
	...
	for (i = 0; i < nummaterials; i++) {
		...
		model->materials[i].diffuse[0] = (float)0.8;
		model->materials[i].diffuse[1] = (float)0.8;
		model->materials[i].diffuse[2] = (float)0.8;
		model->materials[i].diffuse[3] = 1.0;
		model->materials[i].ambient[0] =(float)0.2;
		model->materials[i].ambient[1] = (float)0.2;
		model->materials[i].ambient[2] = (float)0.2;
 ...
 */
GLvoid ObjectModel::Glm::glmWriteMTL(char* modelpath, char* mtllibname)
{
	FILE* file;
	char* dir;
	char* filename;
 ...
 */
GLvoid ObjectModel::Glm::glmFirstPass(FILE* file) 
{
 ...
 * file  - (fopen'd) file descriptor 
 */
GLvoid ObjectModel::Glm::glmSecondPass(FILE* file) 
{
	GLuint  numvertices;        /* number of vertices in model */
	GLuint  numnormals;         /* number of normals in model */
	GLuint  numtexcoords;       /* number of texcoords in model */
	...
	GLMgroup* grp;            /* current group pointer */
	...
	while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'v':               /* v, vn, vt */
			...
			case 'u':
				fgets(buf, sizeof(buf), file);
				sscanf(buf, "%s %s", buf, buf);
				grp->material = material = glmFindMaterial(buf);
				break;
			case 'g':               /* group */
				/* eat up rest of line */
				fgets(buf, sizeof(buf), file);
			...
			case 'f':               /* face */
				...
				if (strstr(buf, "//")) {
					...
					grp->triangles[grp->numtriangles++] = numtriangles;
					numtriangles++;
					while(fscanf(file, "%d//%d", &v, &n) > 0) {
					...
					T(numtriangles).nindices[2] = n;
					grp->triangles[group->numtriangles++] = numtriangles;
					numtriangles++;
					while(fscanf(file, "%d/%d/%d", &v, &t, &n) > 0) {
						T(numtriangles).vindices[0] = T(numtriangles-1).vindices[0];
